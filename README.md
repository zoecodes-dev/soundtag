...
적절한 레퍼런스를 추천한다. 핵심 목표는 "장르 분류"가 아니라 **창의적 조합의 질
평가**다 — 레퍼런스에서 유사도가 너무 높으면 오히려 탈락 신호로 본다.
...
AST(Audio Spectrogram Transformer)로 장르 분류 모델을 5번 학습했다.
가장 높았던 validation accuracy는 **77.6%**. 처음엔 성능이 잘 나온다고 판단했다.

하지만 결과를 뜯어보니, 모델이 학습한 것은 '장르'가 아니라 **'데이터의 출처 유형'**
이었다. 드럼루프(FSLD)와 풀트랙(Deezer)을 장르 라벨링 맞춰 한 데이터셋에 섞었는데,
모델은 구분하기 쉬운 **소스 특성**을 학습했다. 77.6%는 실력이 아니라
**source-type leakage에 의한 가짜 신호**였다.

![accuracy trajectory](experiments/accuracy_trajectory.png)

소스 갭을 한 겹씩 제거하자 정확도는 77.6% → 59% → 47.8% → 43.5% → 46%로 떨어졌다.
**이 하락이 곧 발전이다.** Train accuracy는 5번 내내 99%대에 고정 → 모델은 항상
학습 데이터를 외웠고, validation이 떨어진 건 인위적으로 새던 "소스 단서"가 사라졌기
때문이다. 46%가 이 문제의 진짜 baseline이다.

**근본 원인**은 단일 버그가 아니라 측정 체계의 결함이었다. 5번의 학습이 전부
학습 데이터 분포 내에서만 측정되어, 진짜 목표인 K-pop 성능은 한 번도 평가되지
않았다. **좋은 모델을 만들기 전에, 무엇을 성능이라 부를지부터 잘못 정의돼 있었다.**
...
가짜 신호를 구조적으로 차단하기 위해 세 가지를 도입했다.
...
소스 타입별로 모델을 분리한 것이 핵심 설계 결정이다 — 한 모델에 섞으면
source-type leakage가 재발하기 때문.
...
> 용량·저작권 문제로 git에서 제외한다. Kaggle 데이터셋(`soundtag-hybrid-v3`,
> `soundtag-kpop-previews` 등)과 코드로 재현 가능하다.

---

## 핵심 학습 (Key Learnings)

- **Measurement before training** — 측정 기준이 틀리면 모델 개선은 무의미하다.
  5번의 실패는 전부 측정 체계 결함에서 비롯됐다.
- **Data quality ≫ quantity** — 전문가 태깅 데이터(FSLD)가 검색 기반 수집
  데이터(Deezer)보다 적은 양으로 더 높은 recall을 보였다.
- **Source-type leakage** — 소스가 다른 데이터를 라벨링 맞춰 섞으면, 모델은
  장르가 아니라 데이터 출처를 학습한다. 높은 정확도일수록 의심해야 한다.
- **Falsifiable hypotheses** — 사전 가설 없는 실험은 같은 실수를 보이지 않게
  반복한다.
