...
> **📌 이 문서는 초기 10-stem 분리 비전 시절에 설계한 합성 데이터 파이프라인입니다.**
> 깨끗한 stem에 K-pop 프로덕션 이펙트를 입혀 stem-separation 학습 데이터를 만들려던
> 방향인데, 그 비전 자체는 지금 **보류** 상태입니다. 다만 코드는 그대로 재사용할 수
> 있는 자료라 보존해 둡니다. 지금 살아있는 방향은 [`../README.md`](../README.md)에 있습니다.

## 무엇을 만들려는가

아이디어는 단순합니다. MIDI를 VST로 렌더링한 깨끗한 stem은 너무 깨끗해서 실제
K-pop과 거리가 멉니다. 여기에 실제 K-pop 프로덕션 수준의 이펙트를 한 겹씩 입혀
**"진짜 K-pop 믹스처럼 들리는"** 오디오를 만들고, 그 믹스와 원래의 깨끗한 stem을
한 쌍으로 묶으면 stem-separation 학습 데이터가 됩니다 — 모델 입력은 "지저분한
믹스", 정답은 "이걸 분리해낸 깨끗한 stem"이 되는 셈입니다.

핵심 라이브러리는 `pedalboard`(Spotify), `scipy.signal`, `numpy`, `librosa`입니다.
...
믹스는 트랙 하나하나를 다듬는 데서 시작합니다. 악기마다 K-pop이 그 소리를
어떻게 다루는지가 다르므로, stem별로 전용 이펙트 체인을 따로 짭니다.

### 1-1. Lead Vocal Chain

K-pop 리드 보컬은 밝고 선명하게, 믹스의 맨 앞에, 두껍게 레이어링해서 앉힙니다.
그 인상을 만드는 체인입니다.
...
K-pop 킥은 펀치감과 서브 레이어를 함께 갖고, 동시에 사이드체인의 트리거 역할까지
합니다 — 뒤에서 베이스와 패드를 밀어내는 기준점이 되는 소리입니다.
...
K-pop 베이스는 대부분 808이나 신스 베이스입니다. 깊게 깔리면서도 펀치감을 잃지
않는 게 관건입니다.
...
여기서부터가 K-pop 사운드의 핵심입니다. 킥이 칠 때마다 베이스와 신스가 살짝
주저앉았다 돌아오는 "펌핑" — 이 호흡이 K-pop 특유의 그루브를 만듭니다. 실제
사이드체인 컴프레서가 하는 일을, 트리거 소스(보통 킥)의 엔벨로프를 따 게인
리덕션으로 흉내 냅니다.
...
실제 믹싱은 개별 트랙에서 끝나지 않습니다. 비슷한 트랙끼리 버스(그룹)로 묶어
한 번에 글루 컴프레션을 걸고, 그 묶음을 다시 마스터로 보냅니다 — 개별 트랙 →
버스 → 마스터로 흐르는 이 단계가 소리를 하나로 붙여줍니다.
...
마지막은 마스터링입니다. K-pop 마스터의 인상은 한마디로 밝고, 라우드하고,
펀치감 있게입니다 — 문제 주파수를 도려내는 surgical EQ로 시작해, 글루
컴프레션으로 믹스를 하나로 묶고, 에어를 더한 뒤, 리미터로 라우드니스를
끌어올려 끝맺습니다.
...
K-pop의 스테레오 이미지에는 규칙이 있습니다 — 센터(보컬)는 모노로 단단하게
모으고, 사이드(신스·패드)는 넓게 펼칩니다. 스테레오를 Mid/Side로 분리해
사이드만 부스트하면 이 인상을 만들 수 있습니다.
...
지금까지의 조각들을 한 줄기로 잇습니다. 개별 stem을 각자의 체인으로
프로세싱하고, 킥을 트리거로 사이드체인을 걸고, 버스로 묶고, K-pop 전형의
밸런스(보컬 최전면, 드럼 강하게, 베이스 두껍게, 악기는 뒤)로 합친 뒤,
마스터링과 정규화로 마무리합니다.
...
이 파이프라인의 진짜 가치는 여기서 나옵니다. 모든 파라미터가 랜덤 범위라, 같은
clean stem을 넣어도 매번 다른 믹스가 떨어집니다. 한 곡에서 수십~수백 개의 변형을
찍어낼 수 있다는 뜻입니다.
...
이 설계를 관통하는 생각은 여섯 가지입니다. **모든 파라미터를 랜덤 범위로 둔**
덕에 매번 다른 이펙트 세팅이 나오고, 모델은 다양한 프로덕션 스타일에
일반화됩니다. 믹스 밸런스는 **K-pop 전형**을 따릅니다 — 보컬 최전면, 드럼
강하게, 베이스 두껍게, 악기는 뒤. **사이드체인 시뮬레이션**으로 킥이 베이스·패드를
미는 펌핑을 재현하는 게 핵심이고, **버스 프로세싱**으로 개별 stem → 그룹 →
마스터의 실제 믹싱 흐름을 3단계로 따라갑니다. **Mid/Side**로 센터는 단단하게
사이드는 넓게 잡아 K-pop의 스테레오 이미지를 만들고, 마지막 **마스터링**이 밝고
라우드한 최종 사운드로 끝맺습니다.

이렇게 하면 Slakh2100 한 곡에서 수십~수백 개의 변형이 나옵니다 — 2,100곡 ×
100변형, 약 210,000개의 학습 샘플이 공짜로 만들어집니다.
