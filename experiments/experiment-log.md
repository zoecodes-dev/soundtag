...
> SoundTag의 AST 장르 분류 실험을 처음부터 끝까지 기록한 문서입니다.
> 줄거리는 하나입니다 — validation accuracy 77.6%가 source-type leakage에 의한
> 가짜 신호였음을 알아채고, 데이터의 소스 갭을 한 겹씩 벗겨내며 진짜 성능을
> 쫓아간 과정입니다. 여기서는 점수가 떨어지는 것이 곧 발전이었습니다.

---

## 1. 5번의 학습 — 점수 하락이 곧 진전

다섯 번 모두 동일한 AST 모델(`MIT/ast-finetuned-audioset-10-10-0.4593`)을
fine-tuning했고, Kaggle T4 GPU에서 10 epoch을 돌렸습니다. 데이터 구성만
바꿔가며 학습했습니다.
...
**읽는 법**: Train accuracy는 5번 내내 99%대에 고정돼 있습니다. 모델은 항상
학습 데이터를 완벽히 외웠습니다. Validation만 77.6% → 46%로 떨어졌는데, 이는
성능 저하가 아니라 **모델이 외울 수 있었던 "소스 단서"가 데이터 정리와 함께
사라진 결과**입니다. 77.6%는 가짜였고, 46%가 이 문제의 정직한 baseline입니다.
...
장르는 0.32입니다. 모델은 **"드럼루프냐 full track이냐"는 거의 완벽히
구분**하고, 그 안에서 진짜 장르는 구분하지 못했습니다. 장르 라벨은 같아도 소스
타입이 다른 데이터를 한 데이터셋에 섞었기 때문에, 모델은 구분하기 쉬운 소스
특징을 학습한 것입니다. 77.6%는 소스가 쉽게 갈리는 클래스들이 끌어올린
평균값이었습니다.

### 2-2. K-pop 추론이 보여주는 거울 효과

같은 K-pop 1,874곡을 각 모델에 넣으면 완전히 다른 분포가 나옵니다.
모델이 K-pop을 이해한 게 아니라 각자의 학습 데이터 구성을 비추고 있었던
것입니다.
...
→ 같은 곡, 다른 모델, 완전히 다른 답입니다. 어느 것도 K-pop 자체를 평가한
결과가 아닙니다.
...
이건 코드 한 줄을 고치면 되는 단일 버그가 아니었습니다. 측정 방법 자체가 잘못
놓여 있었습니다.

1. 5번의 학습이 전부 학습 데이터의 Train/Val split **내부**에서만 측정됐습니다.
2. 정작 목표인 K-pop 성능은 단 한 번도 직접 평가되지 않았습니다. (K-pop 추론은
   했지만 ground truth 라벨이 없어 "분포"만 관찰했을 뿐, 정확도는 측정할 수
   없었습니다.)
3. 좋은 모델을 만들기 전에, **무엇을 성능이라 부를지**가 잘못 정의되어
   있었습니다.
...
소스 갭을 데이터로 섞지 않고 모델을 분리해 회피합니다.
...
> git에서 제외합니다. Kaggle 데이터셋(`soundtag-hybrid-v3`,
> `soundtag-kpop-previews` 등)으로 재현 가능합니다.

---

## 6. 한 줄 결론

측정 기준이 틀리면 모델을 아무리 개선해도 의미가 없습니다.
77.6%는 떨어진 것이 아니라, 가짜였던 것이 드러난 것입니다.
